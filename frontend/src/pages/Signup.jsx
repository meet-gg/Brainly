...
const Signup = () => {
  ...
  const getPasswordStrength = () => {
    ...
    if (password.length < 6) return { label: 'Too short', color: 'text-red-500', bg: 'bg-red-500', width: '25%' };
    if (password.length < 8) return { label: 'Weak', color: 'text-orange-500', bg: 'bg-orange-500', width: '50%' };
    if (password.length < 12) return { label: 'Good', color: 'text-yellow-500', bg: 'bg-yellow-500', width: '75%' };
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              onFocus={(e)=>setFieldErrors((ele) =>ele.name ? {...ele, name: null} : ele)}
              className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 ${
                fieldErrors.name ? 'border-red-400 bg-red-50 dark:bg-red-900/20' : 'border-gray-200 dark:border-gray-600'
              }`}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setEmail(e.target.value)}
               onFocus={(e)=>setFieldErrors((ele) =>ele.email ? {...ele, email: null} : ele)}
              className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 ${
          ...
          <div>
            ...
            <input
              ...
              onFocus={(e)=>setFieldErrors((ele) =>ele.password ? {...ele, password: null} : ele)}
              className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 ${
